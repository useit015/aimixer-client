...
function Mix() {
  ...
  const handleSetTopics = async e => {
    ...
    for (let i = 0; i < curBowl.contents.length; ++i) {
      promises.push(
        axios({
          url: `${servers.assets[servers.mode]}/filterTopics`,
          method: 'post',
          data: {
  ...
  const handleMix = async () => {
    ...
    for (let i = 0; i < curBowl.contents.length; ++i) {
      promises.push(
        axios({
          url: `${servers.assets[servers.mode]}/topicsToFacts`,
          method: 'post',
          data: {
            topics: mix.topics,
            link: curBowl.contents[i].link,
  ...
  for (let i = 0; i < curBowl.contents.length; ++i)
    mixLength +=
      typeof curBowl.contents[i].infoLength !== 'undefined'
        ? curBowl.contents[i].infoLength
        : curBowl.contents[i].length;
